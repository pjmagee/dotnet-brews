...
public class CalculatorOperationsLoader(
    ILogger<CalculatorOperationsLoader> logger,
    string pluginsPath
)
{
    public CompositionHost LoadPlugins()
    {
        ...
        var pluginFiles = Directory.GetFiles(
            pluginsPath,
            "Brew.Features.Mef.Operations.*.dll",
...
{
    public IEnumerable<ICalculatorOperation> Operatons =>
        operationsLoader.LoadPlugins().GetExports<ICalculatorOperation>();

    public double Calculate(ICalculatorOperation operation, double arg1, double arg2)
    {
        var result = operation.Operate(arg1, arg2);
        logger.LogInformation(
            "Result of {Operation} operation on {Arg1} and {Arg2} is {Result}",
            operation,
            arg1,
            arg2,
            result
