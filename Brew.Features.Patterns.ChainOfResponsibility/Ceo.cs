...
{
    public Ceo(ILogger<Ceo> logger)
        : base(logger, "Director") { }
    ...
    public override void ProcessRequest(Request request)
    {
        ...
        if (request.Type == RequestType.Large)
        {
            ...
            Logger.LogInformation(
                "[{Title}] ✓ APPROVED - I have final authority for large purchases (> $10,000)",
                Title
            );
        }
        else
        {
            ...
            Logger.LogWarning(
                "[{Title}] ⚠ Unexpected request type - no further handlers in chain",
                Title
            );
        }
    }
