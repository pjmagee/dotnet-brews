        ...
        var brewClassNames = classDecls
            ...
            .Select(
                static (pair, _) =>
                {
                    ...
                    if (symbol is null)
                        return null;
                    var implementsIBrew = symbol.AllInterfaces.Any(i =>
                        string.Equals(i.Name, "IBrew", StringComparison.Ordinal)
                    );
                ...
                var code = CompilationUnit()
                    .WithUsings(
                        new SyntaxList<UsingDirectiveSyntax>(
                            new[] { UsingDirective(IdentifierName(nameof(System))) }
                        )
                    )
                    .WithMembers(
