...
public class Brew : ModuleBase
{
    ...
    )
    {
        ...
        services.AddTransient<
            ...
            GetProductSummariesQueryHandler
        >();
        services.AddTransient<
            IQueryHandler<GetLowStockProductsQuery, IEnumerable<ProductSummary>>,
    ...
    protected override async Task ExecuteAsync(CancellationToken token = default)
    {
        ...
        Logger.LogInformation("");
        Logger.LogInformation(
            "--------------- EXECUTING COMMANDS (Write Operations) ---------------"
        );
        ...
        {
            Logger.LogInformation(
                "  [{Id}] {Name} - Stock: {Stock}, Price: ${Price:F2}, Active: {Active}",
                product.Id,
                product.Name,
                product.Stock,
                product.Price,
        ...
        {
            Logger.LogInformation(
                "  [{Id}] {Name} - Stock: {Stock} (NEEDS REORDER)",
                product.Id,
                product.Name,
                product.Stock
            );
        ...
        Logger.LogInformation(
            "✓ Scalability: read/write stores can be independent (different DBs)"
        );
        Logger.LogInformation(
            "====================================================================="
        );
