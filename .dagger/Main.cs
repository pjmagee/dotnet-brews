...
public class DotnetBrews
{
    ...
    public Directory Format([DefaultPath("/")] Directory directoryArg)
    {
        ...
        var container = Dag.Container()
            ...
            .WithExec(["sh", "-c", "dotnet tool restore && dotnet tool run csharpier -- format ."]);
    ...
    /// <summary>
    /// Release build the solution.
    /// </summary>
    [Function]
    [Check]
    public Container ReleaseBuild([DefaultPath("/")] Directory directoryArg)
    {
        var cache = Dag.CacheVolume("nuget");
        ...
        return Dag.Container()
            ...
            .WithExec(["sh", "-c", "dotnet tool restore && dotnet tool run csharpier -- check ."])
            // build the solution to catch compilation errors
            .WithExec(["dotnet", "build", "Brew.slnx", "-c", "Release"]);
    }

    /// <summary>
    /// Run only CSharpier in check mode
    /// </summary>
    [Function]
    [Check]
    public Container CSharpier([DefaultPath("/")] Directory directoryArg)
    {
        var cache = Dag.CacheVolume("nuget");
    ...
    public async Task<Directory> Ci([DefaultPath("/")] Directory directoryArg)
    {
        ...
        // Workflow 5: Run release-build check function via dagger call
        var daggerCheckJob = Dag.Gha()
            .Job("dagger-check", "release-build", runner: ["ubuntu-latest"]);
