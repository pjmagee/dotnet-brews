...
public class Brew : ModuleBase
{
    protected override Task BeforeAsync(CancellationToken token = default)
    {
        ModuleBase.Logger.LogInformation(
            "================ POOR MAN'S DI (Anti-Pattern) ================"
        );
        ModuleBase.Logger.LogInformation(
            "ConcreteCar directly instantiates ALL dependencies inside its constructor."
        );
        ModuleBase.Logger.LogInformation(
            "Drawbacks: Tight coupling, difficult substitution, harder unit testing, violates SRP & OCP."
        );
    ...
    {
        ModuleBase.Logger.LogInformation(
            "================ CONTAINER-DRIVEN DI (Preferred) ============="
        );
        ModuleBase.Logger.LogInformation("FlexibleCar receives abstractions via DI container.");
        ModuleBase.Logger.LogInformation(
            "Benefits: Substitutability, testability (mock interfaces), configurability, adherence to OCP."
        );
        ...
        ModuleBase.Logger.LogInformation(
            "Anti-Pattern: new ConcreteCar() => implicit hidden dependencies, change requires code modifications."
        );
        ModuleBase.Logger.LogInformation(
            "DI Pattern: container resolves Car with IWheels/IEngine/IChassis => swap implementations without touching consumer."
        );
        ModuleBase.Logger.LogInformation(
            "=============================================================="
        );
